...
while cap.isOpened():
    ...
    for i in range(16):
        ...
        if i % 2 == 0:
            ...
            check_point_condition = False
            check_angle_condition = False
            for (x, y) in zip(x_point_array, y_point_array):
                angle = 2 * 3.14 - math.atan2(y - center[1], x - center[0])
                # atan2は[-π, π]の範囲で角度を返すため、[0, 2π]の範囲に変換します。
                distance = math.sqrt((x - center[0])**2 + (y - center[1])**2)
                check_point_condition = (radius - thickness <= distance and distance <= radius + thickness) or check_point_condition
                check_angle_condition = (angle_start <= angle and angle <= angle_end) or check_angle_condition
